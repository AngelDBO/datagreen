...
function file() {
    ...
    if (arrExtensiones.includes(extension)) {
        document.getElementById("tempFile").innerHTML =
            `<div class="alert alert-primary alert-dismissible pb-2 pt-2" role="alert">
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            data.forEach(element => {
                select = select + `<option value='${element.id}'>${element.nombre}</option>`;
            });
...
function listarUsuarios() {
    $.get('./../controllers/ArchivoController.php?opcion=listarUsuarios', (response) => {
        let data = JSON.parse(response);
        let select = `<option value='' selected disabled>Seleccione</option>`;
        data.forEach(element => {
            select = select + `<option value='${element.id}'>${element.usuario}</option>`;
        });
